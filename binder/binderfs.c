...
static inline struct dentry *binderfs_lookup_one(struct dentry *parent,
												 const char *name,
												 size_t name_len)
{
	return compat_lookup_one(parent, name, name_len);
}
										 ...
										 struct binderfs_device *req)
{
	...
	/* look it up */
	dentry = binderfs_lookup_one(root, name, name_len);
	if (IS_ERR(dentry))
	{
											 ...
											 const char *name)
{
	...
	dentry = binderfs_lookup_one(parent, name, strlen(name));
	if (IS_ERR(dentry))
		return dentry;
...
/* Out-of-tree addition: allow module unload to tear down binderfs. */
